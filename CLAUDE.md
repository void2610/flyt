...
- ホットエッジは画面の辺（上下左右）から選択可能で、デフォルトは無効
...
│   ├── HotEdgeManager.swift   # ホットエッジ設定管理
│   ├── MenuBarManager.swift   # メニューバーアイコン管理
│   ├── PomodoroManager.swift  # ポモドーロタイマーロジック
│   ├── SupabaseClient.swift   # Supabaseクライアント管理
│   ├── AuthManager.swift      # Google認証管理
│   └── SyncManager.swift      # セッションデータ同期管理
├── Utils/
│   └── UserDefaultsKeys.swift # UserDefaultsキー一元管理
├── Views/
...
│   │   ├── HotEdgeSettingsTab.swift     # ホットエッジ設定タブ
│   │   ├── SyncSettingsTab.swift        # クラウド同期設定タブ
│   │   ├── AccessibilitySettingsTab.swift # アクセシビリティ設定タブ
│   │   └── AboutTab.swift               # アプリ情報タブ
│   └── Components/
│       ├── HotKeyRecorderView.swift     # ホットキー記録コンポーネント
│       └── HotEdgeSettingsView.swift    # ホットエッジ設定コンポーネント
```
